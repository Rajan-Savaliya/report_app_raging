...
import { getOneTimeBookingReportAction } from "../../redux/actions/productActions.js";

import Spinner from "react-native-loading-spinner-overlay";

const { width } = Dimensions.get("window");

const OneTimeReport = ({ navigation }) => {
  ...
  const {
    ...
    oneTimeReports,
    oneTimeReportsError,
    oneTimeReportsLoading,
    cardItems,
  } = useSelector((state) => state.productState);

  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      var serviceSelectionList = [];
    ...
    <SafeView bgColor={"#FFFFFF"}>
      ...
      >
        <View style={{ marginBottom: 3, marginLeft: 5 }}>
          <Text style={{ fontSize: 15, color: "#808080" }}>Select Service</Text>
        </View>
        <View>
          <DropDownPicker
      ...
      {oneTimeReportsLoading ? (
        ...
        (Array.isArray(oneTimeReports) && oneTimeReports.length === 0) ? (
        <View
          style={{
            bottom: 30,
            justifyContent: "center",
