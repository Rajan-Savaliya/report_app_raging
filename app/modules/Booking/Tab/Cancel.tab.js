...
const CancelTab = ({ navigation }) => {
  ...
  const renderCardItem = ({ item }) => {
    return (
      <BookingItem navigation={navigation} item={item} typeItem="CANCELLED" />
    );
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      {bookingLoadingItem ? (
        ...
        (Array.isArray(cancelBookingList) && cancelBookingList.length === 0) ? (
        <ScrollView
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={onRefresh} />
          }
                ...
                >
                  ...
                  <View>
                    <Text style={{ top: 10, color: "#000000", fontSize: 17 }}>
                      {bookingError
                        ? bookingError
                        : "No Any Booking Cancel Data Found"}
                    </Text>
                  </View>
                </View>
              ) : (
