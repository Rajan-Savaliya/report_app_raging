...
const BookingTab = ({ navigation }) => {
  ...
  const { completeBookingList, bookingError, bookingLoadingItem } = useSelector(
    (state) => state.productState
  );
  const renderCardItem = ({ item }) => {
    return <BookingItem navigation={navigation} item={item} typeItem="DONE" />;
  };
                ...
                >
                  ...
                  <View>
                    <Text style={{ top: 10, color: "#000000", fontSize: 17 }}>
                      {bookingError ? bookingError : "No Booking Data Found"}
                    </Text>
                  </View>
                </View>
              ) : (
